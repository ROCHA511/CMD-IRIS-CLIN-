...
app.get('/api/download-project', (req, res) => {
  try {
    const archivePath = '/tmp/CMD_IRIS_CLIN_Projeto_Completo.tar.gz';
    try {
      execSync(`tar -czf "${archivePath}" --exclude="node_modules" --exclude=".git" --exclude="dist" --exclude="*.tar.gz" --exclude="*.zip" -C "${process.cwd()}" .`);
    } catch (e) {
      // ignore tar warnings
    }

    if (!fs.existsSync(archivePath)) {
      return res.status(500).json({ error: 'Erro ao gerar o arquivo de backup.' });
    }

    res.download(archivePath, 'CMD_IRIS_CLIN_Projeto_Completo.tar.gz', (err) => {
      if (fs.existsSync(archivePath)) {
...
app.post('/api/copilot/chat', async (req, res) => {
  try {
    ...
    if (!ai) {
      // Realistic pre-generated clinical assistant responses for fallback mode
    ...
    // Build extensive system prompt containing full IRIS AI Enterprise Brain constraints
